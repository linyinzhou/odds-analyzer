...
The dashboard is static-hosted, while its normalized JSON is generated by the refresh job:
...
- `mismatch_history` is upserted by batch and fixture and drives the Mismatch panel.
- `checker_history` is upserted by batch and fixture and drives the Checker panel.
- Each match should include a concrete `prediction` object with market, pick, and confidence.
- The evening job rebuilds query-time fixtures, fundamentals, European odds, Asian handicap, Sporttery data when available, and concrete predictions in this shape.
