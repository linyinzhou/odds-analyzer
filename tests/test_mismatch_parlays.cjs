...
assert.equal(calculate([confident, match('double', 1.73)], {mixed:true, minConfidence:80}).status, 'insufficient');
...
assert.match(node('#parlayResults').innerHTML, /暂不能生成组合/);
